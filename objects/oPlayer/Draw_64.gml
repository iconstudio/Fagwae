...
if hp > 0 {
	...
	for (i = 0; i < out_surfh; i += 2)
		draw_surface_part_ext(out_surf, 0, i, out_surfw, 2, dx - out_x - 1, dy - out_y + i - 1, 1, -1, dcol, 0.5 + lengthdir_x(0.4, out_rotation + i * 7))
}
...
if hp > 0 {
	dcol = make_color_hsv(hp * 25 - 15, 216, 255)
	for (i = 0; i < out_surfh; i += 2)
		draw_surface_part_ext(out_surf, 0, i, out_surfw, 2, dx - out_x - 1, dy - out_y + i - 1, 1, -1, dcol, 0.2 + lengthdir_x(0.2, out_rotation / 3 + i * 7))
}
