/// @description enemy_init_frame()
/// @function enemy_init_frame

enemy_register(enemy_square_1, oFrameSquare, "STEEL FRAME", sFrameSquare, sFrameSquare, 
30, 250, 2, 0, 0, 0, 0)

enemy_register(enemy_square_2, oFrameTurretSquare, "STEEL FRAME", sFrameSquare, sFrameSquare, 
33, 400, 2, 0, 0, 0, 0)

enemy_register(enemy_line_1, oFrameLine1, "STEEL FRAME", sFrameLine, sFrameLine, 
70, 650, 2, 0, 0, 0, 0)

enemy_register(enemy_line_2, oFrameLine2, "STEEL FRAME", sFrameLine, sFrameLine, 
50, 650, 2, 0, 0, 0, 0)

enemy_register(enemy_board_1, noone, "BOARD", noone, noone, 
121, 700, 2, 0, 0, 0, 0)

enemy_register(enemy_box_1, oFrameBulletBox, "BOX", sFrameBulletBox, sFrameBulletBox, 
60, 400, 2, 0, 0, 0, 0)

enemy_register(enemy_mutation_1, noone, "STEEL OCTAGON", sFrameOctagon, sFrameOctagon, 
17, 300, 1, 0, 0, 0, 0)

enemy_register(enemy_mutation_2, noone, "STEEL OCTAGON", sFrameOctagon, sFrameOctagon, 
12, 300, 1, 0, 0, 0, 0)

enemy_register(enemy_piece_1, oFramePiece, "STEEL FRAME", sFrameJoint, sFrameJoint, 
20, 300, 1, 0, 0, 0, 0)

enemy_register(enemy_piece_2, oFramePiece, "STEEL FRAME", sFrameJoint, sFrameJoint, 
30, 340, 2, 1, 0, 0, 0)

enemy_register(enemy_joint_1, oFrameJoint, "SKELETON", sFrameJoint, sFrameJoint, 
20, 250, 1, 0, 0, 0, 0)

enemy_register(enemy_joint_2, oFrameJoint, "SKELETON", sFrameJoint, sFrameJoint, 
20, 250, 1, 0, 0, 0, 0)

enemy_register(enemy_joint_3, oFrameJointMod, "SKELETON", sFrameJoint, sFrameJoint, 
10, 150, 1, 0, 0, 0, 0)

enemy_register(enemy_abomination_1, oFrameJointOctopod, "ABOMINATION", sFrameCore, sFrameCore, 
90, 700, 2, 0, 0, 0, 0)

enemy_register(enemy_abomination_2, noone, "ABOMINATION", sFrameCore, sFrameCore, 
90, 700, 2, 0, 0, 0, 0)

enemy_register(enemy_abomination_3, noone, "ABOMINATION", sFrameCore, sFrameCore, 
80, 700, 2, 0, 0, 0, 0)


// bosses
