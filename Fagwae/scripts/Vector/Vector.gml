...
#region public
	...
	static at = function(Index) {
		if inner_size <= abs(Index)
			return undefined
		elif Index < 0
			return raw[inner_size + Index]
		else
			return raw[Index]
	}
	...
	///@function back()
	static back = function() { return at(-1) }
	...
	static insert_at = function(Index, Value) {
		array_insert(raw, Index, Value)
		inner_size = array_length(raw)
		return self
	}
	...
	///@function sort_builtin(predicate)
	static sort_builtin = function(Param) { array_sort(raw, Param) }
...
#region private
	...
	///@function 
	static _Under_resize = function(Size) {
		inner_size = Size
		modified = false
		array_resize(raw, Size)
	}
