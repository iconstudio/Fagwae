/// @description Drawing the player
var dx = round(x)
var dy = round(y)
var dcol, i
...
draw_sprite_ext(sPlayerArm, 2, out_a4x, 3 + out_ay, 1, 1, angle_player - angle_arm_right, $ffffff, 1)
...
if 0 < hp {
	...
	for (i = 0; i < out_surfh; i += 2)
		draw_surface_part_ext(out_surf, 0, i, out_surfw, 2, dx - out_x - 1, dy - out_y + i, 1, -1, dcol, 0.5 + lengthdir_x(0.4, out_rotation + i * 6))
}
...
if 0 < hp {
	dcol = make_color_hsv(hp * 25 - 15, 216, 255)
	for (i = 0; i < out_surfh; i += 2)
		draw_surface_part_ext(out_surf, 0, i, out_surfw, 2, dx - out_x - 1, dy - out_y + i, 1, -1, dcol, 0.2 + lengthdir_x(0.2, out_rotation * 0.333 + i * 6))
}
