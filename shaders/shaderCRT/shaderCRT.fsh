...
vec4 scanlineWeights(float dist, vec4 col) {
    vec4 wid = 2.0 + 4.0 * pow(col, vec4(4.0));
    vec4 weights = vec4(dist * 3.333333);                
    return 0.31 * exp(-pow(abs(weights * sqrt(2.0 / wid)), wid)) / (0.18 + 0.06 * wid);
}
...
void main() {
    ...
    vec4 weights  = scanlineWeights(abs(uv_ratio.y), col) * 1.0;
    //vec4 weights2 = scanlineWeights(1.0 - uv_ratio.y, col2) * 1.0;
